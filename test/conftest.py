...
def _make_cfg(target: str) -> CxPlat:
    ...
    if target == "linux":
        return CxPlat(windows=False, directory="castxml_linux",  native=(not is_native_windows))
    elif target == "windows":
        return CxPlat(windows=True,  directory="castxml_windows", native=is_native_windows)
    else:
