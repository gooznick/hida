...
def is_system_header(file_name: str) -> bool:
    system_paths = [
        "/usr/", "/x86_64-linux-gnu/", "/castxml/",
        "<built-in>", "<command-line>", "<builtin>"
    ]
    ...
    for elem in elements.values():
        if elem.tag in ('Struct', 'Class', 'Enumeration', 'Typedef', 'Union'):
            ...
            if elem.tag in ('Struct', 'Class'):
                ...
                definitions.append(UnionDefinition(
                    name=full_name,
                    alignment=int(elem.attrib['align']) // CHAR_BIT,
                    size=int(elem.attrib['size']) // CHAR_BIT,
                    fields=parse_fields(elem, elements)
                ...
                definitions.append(EnumDefinition(
                    name=full_name,
                    size=int(elem.attrib['size']) // CHAR_BIT,
                    enums=enums
                ))
                ...
                typedef_def = typedef_type_elem.attrib.get('name', 'unknown') if typedef_type_elem else 'unknown'
                definitions.append(TypedefDefinition(
                    name=full_name,
...
def print_structure(definitions: List, name: str):
    for definition in definitions:
        if isinstance(definition, ClassDefinition) and definition.name.endswith(name):
            print(f"Structure {definition.name}: size={definition.size} bytes, alignment={definition.alignment} bytes")
            for field in definition.fields:
                print(f"  {field.name}: offset={field.bitoffset} bits, size={field.size_in_bits} bits, type={field.type}, bitfield={field.bitfield}")
