...
def generate_python_code_from_definitions(definitions: List[TypeBase], alignment=None, assert_size=True) -> str:
    lines = [
        "# fmt: off",
    ...
    for d in definitions:
        if isinstance(d, (ClassDefinition, UnionDefinition)):
            ...
            lines.append(f"{to_python_name(d)} = {repr(d.value)}  # type: {to_python_name(d.type)}")
        else:
...
def generate_and_verify(definitions, filename, verify_size=False):
    ...
    if verify_size:
        verify_struct_sizes([d for d in definitions if isinstance(d, (ClassDefinition, UnionDefinition))], module)
